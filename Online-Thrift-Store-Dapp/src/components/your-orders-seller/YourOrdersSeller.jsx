...
class YourOrdersSeller extends Component {
  static propTypes = {
  ...
  render() {
    ...
    let ORDERS = this.props.orders.filter(({ product }) => !product.name.includes("M1 MacBook"));
          ...
          renderItem={item => (
            <List.Item
              actions={[
                <IconText
                  type="check-circle-o"
                  text={view === 'seller' ? 'Accept' : 'Dispatched'}
...
export default connect(mapStateToProps, orderActionsCreator)(YourOrdersSeller);